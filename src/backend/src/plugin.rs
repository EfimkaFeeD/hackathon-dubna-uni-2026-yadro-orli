...
pub fn init_plugin(plugin_path: &str) -> anyhow::Result<()> {
    unsafe {
        ...
        PLUGIN_LIB.set(lib).map_err(|_| anyhow::anyhow!("Библиотека уже загружена"))?;
    }
    Ok(())
}
...
impl AudioProcessor {
    pub fn new(spec: &AudioSpec) -> anyhow::Result<Self> {
        let lib = PLUGIN_LIB.get().ok_or_else(|| anyhow::anyhow!("Библиотека не инициализирована"))?;
        ...
        unsafe {
            ...
            if handle.is_null() {
                return Err(anyhow::anyhow!("C++ код вернул нулевой указатель (ошибка инициализации)"));
            }
